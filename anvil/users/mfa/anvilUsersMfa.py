def add_mfa_method(password, mfa_method, clear_existing=False):
    """Add an MFA method to the current user by passing the user’s password and the mfa method, optionally clearing
    all existing methods. """
    pass


def configure_mfa_with_form(allow_cancel=False):
    """Display a form for the user to configure 2-factor authentication.allow_cancel: if True, the signup form has a
    Cancel button that the user can use to dismiss the form. """
    pass
...
def get_fido_mfa_login(email_address, password):
    """Generate a WebAuthn challenge that the given user can use to log in with a previously registered hardware
    token. """
    pass


def get_totp_mfa_login(code):
    """Get an MFA login object representing a TOTP login code. This can be passed to the login_with_email function as
    the mfa argument. """
    pass


def mfa_login_with_form(email_address, password):
    """Display a form to collect two-factor authentication credentials from the user currently logging in by passing
    the function their email and password. """
    pass
